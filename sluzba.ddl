...
CREATE TABLE radnik (
    ...
    zan           VARCHAR2(15 CHAR) NOT NULL
);

ALTER TABLE radnik
    ADD CONSTRAINT ch_inh_radnik CHECK ( zan IN ( 'Radnik', 'Radnik_u_SZUK', 'Upravnik' ) );
...
ALTER TABLE radnik_u_szuk
    ADD CONSTRAINT radnik_u_szuk_radnik_fk FOREIGN KEY ( sifra_rad )
...
ALTER TABLE upravnik
    ADD CONSTRAINT upravnik_radnik_fk FOREIGN KEY ( sifra_rad )
        REFERENCES radnik ( sifra_rad );

CREATE OR REPLACE TRIGGER arc_fkarc_1_upravnik BEFORE
    INSERT OR UPDATE OF sifra_rad ON upravnik
    FOR EACH ROW
DECLARE
    d VARCHAR2(15 CHAR);
BEGIN
    ...
    FROM
        radnik a
    WHERE
        a.sifra_rad = :new.sifra_rad;

    IF ( d IS NULL OR d <> 'Upravnik' ) THEN
        raise_application_error(-20223,
                               'FK Upravnik_Radnik_FK in Table Upravnik violates Arc constraint on Table Radnik - discriminator column ZAN doesn''t have value ''Upravnik''');
    END IF;
...
CREATE OR REPLACE TRIGGER arc_fkarc_1_radnik_u_szuk BEFORE
    INSERT OR UPDATE OF sifra_rad ON radnik_u_szuk
    FOR EACH ROW
DECLARE
    d VARCHAR2(15 CHAR);
BEGIN
    ...
    IF ( d IS NULL OR d <> 'Radnik_u_SZUK' ) THEN
        raise_application_error(-20223,
                               'FK Radnik_u_SZUK_Radnik_FK in Table Radnik_u_SZUK violates Arc constraint on Table Radnik - discriminator column ZAN doesn''t have value ''Radnik_u_SZUK''');
    END IF;
